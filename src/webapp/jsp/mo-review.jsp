    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-2">
            <div class="flex gap-2">
                <a href="review?status=all&sort=${sortOrder}" class="px-3 py-1 rounded-lg ${filterStatus == 'all' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700'}">All (${pendingCount + acceptedCount + rejectedCount})</a>
            ...
            <div class="flex items-center gap-2 text-sm">
                ...
                <a href="review?status=${filterStatus}&sort=desc" class="px-2 py-1 rounded-lg ${sortOrder == 'desc' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700'}">Newest</a>
                <a href="review?status=${filterStatus}&sort=asc" class="px-2 py-1 rounded-lg ${sortOrder == 'asc' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700'}">Oldest</a>
            </div>
        </div>
            ...
            <c:forEach var="app" items="${apps}">
                <div class="bg-white p-6 rounded-xl shadow-md flex justify-between items-start">
                    ...
                    <div class="flex flex-col gap-3 items-end">
                        ...
                        <c:if test="${app.status != 'pending'}">
                            <span class="px-3 py-1 rounded-full text-xs font-bold uppercase ${app.status == 'accepted' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
                                ${app.status}
                            </span>
                        </c:if>
                    </div>
